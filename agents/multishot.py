"""Multi-shot agent using iterative function calling with visual feedback."""
...
class MultiShotAgent(Agent):
    """
    Multi-shot agent with iterative function calling and visual feedback.

    Tools available:
    - update_spec: Modify text, layout, colors, positions, and all spec properties
    - update_image: Edit image files using AI image editing (Kontext)
    - update_svg: Regenerate SVG files using AI (Gemini)

    Makes multiple LLM calls in a loop, re-rendering and viewing the design after each iteration.
    """
    ...
    def edit(self, spec_path: Path, instruction: str, output_path: Path) -> Path:
        """Edit a design spec using iterative function calling with visual feedback."""
        ...
        edited_spec = self._apply_edit_with_tools(instruction, output_path)

        # Final save
        self.log(f"Saving final spec to {output_path}")
    ...
    def _load_source_image(self, spec_path: Path) -> str:
        ...
        if not render_path.exists():
            raise FileNotFoundError(
                ...
                f"MultiShotAgent requires render.png to exist for image editing.\n"
    ...
    def _render_design(self, saved_path: Path, spec: Spec) -> str | None:
        """Render design to PNG and return data URL. Returns None on failure."""
        ...
        try:
            ...
            # Return data URL for visual feedback
            return _to_data_url(render_output)
        except Exception as e:
            self.log(f"Warning: Failed to render: {e}")
            return None

    def _apply_edit_with_tools(self, instruction: str, output_path: Path) -> Spec:
        """Apply edits using iterative function calling with visual feedback."""
        messages = [
            {"role": "system", "content": self._build_system_prompt()}
        ]

        tools = self._build_tool_definitions()
        current_render_url = None

        # Iterative loop with re-rendering
        for iteration in range(self.max_iterations):
            self.log(f"Iteration {iteration + 1}/{self.max_iterations}")
            self._append_to_edit_log(f"\n{'='*80}\nITERATION {iteration + 1}/{self.max_iterations}\n{'='*80}\n")

            # Build user prompt with current state and optional visual feedback
            user_prompt = self._build_user_prompt(instruction, iteration, current_render_url)
            ...
            if hasattr(assistant_message, 'tool_calls') and assistant_message.tool_calls:
                ...
                # Render after tool execution to get visual feedback
                saved_path = self.save_spec(self.current_spec, output_path)
                current_render_url = self._render_design(saved_path, self.current_spec)

                if current_render_url:
                    self._append_to_edit_log(f"\n✓ Rendered design for visual feedback\n")
            else:
                self.log("No tool calls made - agent finished")
                self._append_to_edit_log("\nNo tool calls made - agent finished\n")
                break
...
This is an ITERATIVE process - you will see the results of your edits rendered visually after each iteration, allowing you to refine your changes.
...
ITERATIVE WORKFLOW:
1. First iteration: Analyze the spec and make edits
2. After your edits, the design will be rendered and shown to you
3. Next iterations: Review the rendered result, check if edits are correct, and refine as needed
4. When satisfied, respond without making tool calls to finish

IMPORTANT:
- When using update_spec, return the COMPLETE spec with ALL fields
- Preserve all nodes you're not modifying
- Make sure your JSON is valid
- After seeing the rendered result, you can make additional refinements
- Don't add \\n to text nodes- they should wrap naturally due to the width of the node
"""

    def _build_user_prompt(self, instruction: str, iteration: int, render_url: str | None) -> str | list:
        """Build user prompt with spec and optional visual feedback."""
        spec_json = json.dumps(self.current_spec.model_dump(), indent=2)

        if iteration == 0:
            # First iteration: just spec and instruction
...
Analyze the current spec and the user's instruction. Determine which tool(s) to use and call them to make the requested edits."""
            ...
            # Subsequent iterations: include rendered image for visual feedback
            content = [
                {
                    "type": "text",
...
Here is the rendered result after your previous edits:"""
                }
            ]
            ...
            if render_url:
                ...
                content.append({
                    "type": "text",
                    "text": "Review the rendered design. Are the edits correct? If refinements are needed, call the appropriate tools. If satisfied, respond without tool calls."
                })
            else:
                content.append({
                    "type": "text",
                    "text": "(Render failed - working from spec only. Make any needed refinements or finish.)"
                })
    ...
    def _init_edit_log(self, instruction: str):
        ...
        header = f"""{'='*80}
MULTISHOT AGENT EDIT LOG (Iterative with Visual Feedback)
{'='*80}
