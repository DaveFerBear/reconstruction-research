...
class SingleShotAgent(Agent):
    """
    Single-shot agent with function calling to modify images/SVGs with closed-loop captioning.
    ...
    Makes a single LLM call that can invoke multiple tools in parallel, and then renders the design.
    """
    ...
    def _load_source_image(self, spec_path: Path) -> str:
        ...
        if not render_path.exists():
            raise FileNotFoundError(
                ...
                f"SingleShotAgent requires render.png to exist for image editing.\n"
...
Analyze the current spec and the user's instruction. Determine which tool(s) to use and call them to make the requested edits."""
    ...
    def _init_edit_log(self, instruction: str):
        ...
        header = f"""{'='*80}
SINGLESHOT AGENT EDIT LOG (Single-Shot)
{'='*80}
