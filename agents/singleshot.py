"""Single-shot agent that provides the rendered original design as context."""
...
class SingleShotAgent(Agent):
    """
    Single-shot design editing agent with visual context.

    Uses an LLM (Gemini) to edit design specs based on text instructions,
    while providing a rendered preview of the original design as a single example.
    """
    ...
    def __init__(
        ...
        """
        Initialize the single-shot agent.
    ...
    def edit(self, spec_path: Path, instruction: str, output_path: Path) -> Path:
        """
        Edit a design spec using single-shot LLM prompting with rendered context.
        ...
        # Determine asset directory (look for assets in same directory as spec)
        asset_dir = spec_path.parent

        edited_spec = self._apply_edit(spec, instruction, asset_dir)

        self.log(f"Saving edited spec to {output_path}")
        return self.save_spec(edited_spec, output_path)

    def _apply_edit(self, spec: Spec, instruction: str, asset_dir: Path = None) -> Spec:
        """
        Apply an edit instruction to a spec using the LLM with visual context.
        ...
        self.log("Rendering original design for context...")
        ...
        # Build the prompt
        prompt_text = self._build_prompt(spec_json, instruction)

        self.log("Calling LLM with rendered context...")
    ...
    def _build_prompt(self, spec_json: str, instruction: str) -> str:
        ...
        """
        return f"""You are a design editing assistant. You will be given:
1. A rendered preview of the current design (as an image)
2. The design specification in JSON format
3. An instruction to modify it
...
Your task:
1. Look at the rendered preview to understand the current design visually
2. Read the design spec to understand the structure
3. Apply the requested modification to the spec
4. Return ONLY the modified JSON spec, with no additional text or explanation
